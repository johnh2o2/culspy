...
culspy = Extension('_culspy', ['culsp_wrap.cpp', 'periodogram.cpp', 'culsp.cu'],
		library_dirs=[CUDA['lib64']],
		include_dirs=[CUDA['include']],
		define_macros=[('BLOCK_SIZE', 256), ('main', 'oldmain')],
		libraries=['argtable2', 'm', 'cudart' ],
		extra_compile_args={'gcc': [],
                     'nvcc': ['-arch=sm_52', '--ptxas-options=-v', '-c', '--compiler-options', "'-fPIC'"]})

if find_in_path('swig', os.environ['PATH']):
    subprocess.check_call('swig -python -c++ -o culsp_wrap.cpp culsp.i', shell=True)
else:
